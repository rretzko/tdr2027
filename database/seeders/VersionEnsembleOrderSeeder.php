...
class VersionEnsembleOrderSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * Reads from a local-only CSV export (database/seeders/data/version_ensemble_order.csv)
    ...
    {
        $rows = $this->readCsv('version_ensemble_order.csv');

        if ($rows === []) {
            return;
        }

        DB::table('version_ensemble_order')->upsert(
            $rows,
            ['id'],
            ['version_id', 'ensemble_id', 'order_by', 'updated_at']
        );
    }
    ...
    private function readCsv(string $filename): array
    {
        ...
        if (! file_exists($path)) {
            $this->command->warn("VersionEnsembleOrderSeeder skipped {$filename}: {$path} not found.");
