...
class VersionRoomSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * Reads from a local-only CSV export (database/seeders/data/version_rooms.csv)
    ...
    {
        $rows = $this->readCsv('version_rooms.csv');

        if ($rows === []) {
            return;
        }

        DB::table('version_rooms')->upsert(
            $rows,
            ['id'],
            ['version_id', 'name', 'tolerance', 'order_by', 'updated_at']
        );
    }
    ...
    private function readCsv(string $filename): array
    {
        ...
        if (! file_exists($path)) {
            $this->command->warn("VersionRoomSeeder skipped {$filename}: {$path} not found.");
        ...
        while (($data = fgetcsv($handle)) !== false) {
            ...
            $rows[] = [
                ...
                'name' => trim($row['name']),
                'tolerance' => $this->nullableInt($row['tolerance'] ?? ''),
