...
## 7. Operational note, 2026-08-25 — encrypted credential columns are tied to the writing environment's `APP_KEY`

A second, more serious instance of the same bug class as §4 step 7's "real credential-storage bug": user 324 was granted Event Manager access to a Version of Event 25 (NJMEA) and immediately hit `DecryptException: The MAC is invalid` opening Configure — not just on `event_id = 25`, but on Event 9 (CJMEA/Square) too, neither of which had been touched that day. Root cause: **`event_epayment_configs.secret`/`webhook_signature_key` for every row in the table had been written by running seed/tinker/test code locally against a database connection pointed at production**, back during this feature's original build (2026-08-13/14) — so every one of them was encrypted under the *local* environment's `APP_KEY`, not production's. It looked, for most of the investigation, like isolated row corruption (compounded by the DB export/import step in §4 step 7's earlier fix, which moved ciphertext bytes without re-encrypting them) before the actual cause — two genuinely different `APP_KEY` values — was confirmed directly.

**Fixed** by decrypting every row locally (against local's key) and re-encrypting each with production's real key, computed with a one-off `Illuminate\Encryption\Encrypter` instance rather than the model's cast, then applied via plain `UPDATE` statements run against production directly.

**Going forward, per [[project_local_prod_app_key_mismatch]] in memory:**
- Local and production `APP_KEY` are different by design and will stay that way (production's protects live vendor secrets; syncing them would extend that exposure to a local dev machine) — do not "fix" this by copying one `APP_KEY` into the other environment's `.env`.
- Never write to `event_epayment_configs.secret`/`webhook_signature_key` (or `epayment_credentials.secret`) from a process running under the *other* environment's `APP_KEY` — that includes local tinker/seed/artisan commands pointed at production's database, and it includes importing a DB dump from one environment into the other. Either always encrypts under the wrong key for the receiving environment.
- The only two safe ways to set these columns: (1) through that environment's own running app — i.e. an Event Manager entering real values via the live Payments tab (§4 step 9) is exactly this, which is why that tab exists; or (2) a script executed *inside* that environment's own runtime (e.g. `vapor tinker <environment>`), never a local session merely pointed at that environment's database.
- Testing credential entry locally and entering the real credentials in production are two independent actions on two independent rows — there is nothing to migrate between them, and no reason to delete the local test row afterward.
