...
**`candidate_payments` table** — new, this phase, schema/report-only (no writer, see above):
...
**Not built this phase (flag, don't scaffold):** the `candidate_payments` writer / any candidate-facing e-payment flow (blocked on the not-yet-built `epayment_credentials` integration, §5.2); the `teacher_payments.payment_type = Electronic` writer (same blocker, teacher side); real room↔Candidate assignment (blocks true Adjudication Backup/Registration Cards content — §7 territory); a re-send/undo path for an already-sent batch confirmation email.
...
`Check | PurchaseOrder | Cash | Other | Electronic` — see §5.10 (`teacher_payments`, `candidate_payments`). `Electronic` is a reserved case on both tables this phase — no writer exists yet on either side, blocked on the not-yet-built `epayment_credentials` (§5.2) integration.
