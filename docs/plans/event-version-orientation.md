...
Still not built (this section's own PDFs/results pages, its own dedicated "Close Audition" button, and any results-release gate on the teacher side) — remains §7 reference-only. One directly-related piece jumped ahead of it, though: closing a Version is already possible today via `VersionEdit`'s general Status field (the only place `versions.status` is set anywhere in the app — Tab Room Module.docx's own Close Audition description, "close the audition and version," describes exactly that one action), and saving that field as Closed now automatically snapshots Ensemble Cut-offs' current counts into `EnsembleHistory` (§9 item 28) — deliberately *not* a separate button on the Ensemble Cut-offs page, so a reopen-correct-reclose cycle can't leave a stale snapshot. This is the full extent of what's built against this section; the results/PDF machinery itself is untouched.
